<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('disbursements.update', $disbursement->id) }}" method="POST" id="disbursementForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <select name="customer_id" id="customer_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                                ...
                                @foreach($customers as $customer)
                                    <option value="{{ $customer->id }}" {{ old('customer_id', $disbursement->customer_id) == $customer->id ? 'selected' : '' }}>
                                        {{ $customer->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" name="disbursement_date" id="disbursement_date" value="{{ old('disbursement_date', $disbursement->disbursement_date->format('Y-m-d')) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                        ...
                        <div class="mb-4">
                            ...
                            <div id="productsContainer">
                                @foreach($disbursement->items as $index => $item)
    ...
    <script>
        let productCount = {{ count($disbursement->items) }};
        ...
        $(document).ready(function() {
            ...
            $(document).on('change', '.product-select', function() {
                ...
                const availableQuantity = parseFloat(selectedOption.data('available'));
                
                quantityInput.attr('max', availableQuantity);
                if (parseFloat(quantityInput.val()) > availableQuantity) {
